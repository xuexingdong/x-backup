...
import com.xuexingdong.x.common.http.XHttp;
import com.xuexingdong.x.common.serialization.XSerialization;
import com.xuexingdong.x.common.utils.XRegexUtils;
...
public class TmallQRCodeServiceImpl implements QRCodeService {
    ...
    public static final Map<String, QRCodeScanStatus> QRCODE_SCAN_STATUS_MAP = new HashMap<String, QRCodeScanStatus>() {
        {
    ...
    public QRCodeScanStatus getScanStatus(String token) {
        ...
        if (scanStatus == QRCodeScanStatus.CONFIRM) {
            ...
            stringRedisTemplate.opsForList().leftPush(TMALL_COOKIES_POOL_REDIS_KEY, XSerialization.toJSON(cookies));
        }
        return scanStatus;
    }
    ...
    public Optional<String> getQRCodeRedirectUri(String token) {
        if (BooleanUtils.isTrue(stringRedisTemplate.hasKey(token))) {
